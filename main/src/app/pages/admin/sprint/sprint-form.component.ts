...
export class SprintFormAdminComponent implements OnInit, OnDestroy {
  ...
  initForm(): void {
    this.sprintForm = this.fb.group({
      ...
      // ADDED: Initialize the 'urgent' form control with a default value (e.g., false)
      urgent: [false] // Default to false
    }, {
      validator: this.sprintDatesValidator()
    });
  }

  // Custom validator for sprint dates
      ...
      .subscribe({
        next: (sprint: Sprint) => {
          ...
          this.sprintForm.patchValue({
            ...
            etudiantsAffectes: sprint.etudiantsAffectes || [],
            // ADDED: Patch the 'urgent' field during loading
            urgent: sprint.isUrgent !== undefined ? sprint.isUrgent : false // Provide a default if not present
          }, { emitEvent: false });
  ...
  onSubmit(): void {
    ...
    const commonSprintProperties = {
      ...
      etudiantsAffectes: formValue.etudiantsAffectes || [],
      urgent: formValue.urgent // ADDED: Include the 'urgent' value from the form
    };
