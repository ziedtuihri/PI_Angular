...
export class CreateQuizComponent {
   offerId!: number;
    ...
    saveQuiz(): void {
      ...
      this.quizService.createQuiz(this.offerId, quiz2).pipe(
        tap(savedQuiz => {
  ...
  const dialogRef = this.dialog.open(AiQuizDialogComponent);
  dialogRef.afterClosed().subscribe((result) => {
    if (result) {
      this.callAIQuizGeneration(result.count, result.prompt);
    }
  });
}
...
callAIQuizGeneration(count: number, prompt?: string): void {
  const offerDescription = this.offer?.description || '';
  this.aiGenerating = true;
  this.rotateLoaderMessages();

  this.quizService.generateAIQuestions({ description: offerDescription, prompt, count })
    .pipe(finalize(() => {
    ...
    .subscribe((questions: any[]) => {
      ...
      questions.forEach((q) => {
        ...
        const questionGroup = this.fb.group({
          content: [q.question, Validators.required],
...
private rotateLoaderMessages(): void {
  let index = 0;
  this.aiLoaderMessage = this.loaderMessages[0];
  this.loaderInterval = setInterval(() => {
    index = (index + 1) % this.loaderMessages.length;
    this.aiLoaderMessage = this.loaderMessages[index];
  }, 2000);
}
...
private loaderMessages = [
  ...
  'Choosing the toughest multiple choice...',
  'Grading imaginary papers...',
  'Assembling neurons...',
  'Avoiding trick questions...',
  'Firing up the AI brain...',
  'Dusting off textbooks...',
  'Checking answer keys twice...',
];
